...
The CRS v4 REST API is the contract between this SPA and CRS. CRS owns it: it
publishes a drift-gated spec at
`components-registry-service-server/src/main/resources/openapi/v4.json` (CRS
TD-003). Portal does not hand-maintain types against it — it **vendors** that
spec and generates TypeScript from it:

- `frontend/src/lib/api/v4.json` — vendored copy of the CRS spec.
- `frontend/src/lib/api/schema.d.ts` — generated by `npm run generate-types`
  (`openapi-typescript`); committed.

Two CI gates (in [`.github/workflows/merge-gate.yml`](.github/workflows/merge-gate.yml))
keep this honest:

1. `generate-types:check` — `schema.d.ts` matches the vendored `v4.json`.
2. `vendor-spec:check` — the vendored `v4.json` matches CRS's published spec at
   a **pinned CRS ref**, so the contract can't silently drift between the repos.

**Refresh flow** — when CRS's contract changes, the staleness gate fails. Refresh
both files in one step (requires the [`gh`](https://cli.github.com) CLI):

```sh
cd frontend
npm run vendor-spec          # fetch CRS spec at the pinned ref → v4.json → regenerate types
git add src/lib/api/v4.json src/lib/api/schema.d.ts
git commit -m "chore(openapi): re-vendor v4.json from CRS <ref>"
```
