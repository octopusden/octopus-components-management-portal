...
| `GET` | `/rest/api/4/info` | Anonymous on **both** sides — Portal `permitAll` + CRS `permitAll`. | CRS `InfoControllerV4` (proxied via `/rest/**` Gateway route). Contract: CRS [SYS-033](https://github.com/octopusden/octopus-components-registry-service/blob/main/docs/registry/requirements-common.md). |
