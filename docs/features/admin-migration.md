...
- **Pod restart during RUNNING:** the next `GET /admin/migrate/job` returns 404 (state was in-memory). The SPA falls back to IDLE. The user must re-run. Tracked in CRS [MIG-028](https://github.com/octopusden/octopus-components-registry-service/blob/main/docs/registry/requirements-migration.md).
...
| Backend | Class-level `@PreAuthorize("@permissionEvaluator.canImport()")` on `AdminControllerV4` + `WebSecurityConfig` `/rest/api/4/**` requires JWT. | The actual gate. CRS [MIG-024](https://github.com/octopusden/octopus-components-registry-service/blob/main/docs/registry/requirements-migration.md). |
