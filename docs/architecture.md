...
| Anonymous build-info | [`src/main/kotlin/.../controller/PortalInfoController.kt`](../src/main/kotlin/org/octopusden/octopus/components/portal/controller/PortalInfoController.kt) | `GET /portal/info` → `{name, version}` from Spring Boot `BuildProperties`, plus optional `environmentLabel` (`portal.environment-label` — per env via service-config, or the `PORTAL_ENVIRONMENT_LABEL` env var; e.g. `TEST INSTANCE` on QA) rendered as a full-width warning banner in the SPA header; key omitted when unset, so prod keeps the original body. |
