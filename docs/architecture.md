...
| Anonymous build-info | [`src/main/kotlin/.../controller/PortalInfoController.kt`](../src/main/kotlin/org/octopusden/octopus/components/portal/controller/PortalInfoController.kt) | `GET /portal/info` → `{name, version}` from Spring Boot `BuildProperties`. |
