...
This work was previously tracked as a sub-bullet under step 8 of [`docs/onboarding/components-management-portal.md`](../onboarding/components-management-portal.md). It was broken out into its own tech-debt entry because it is a one-off operational deliverable rather than an onboarding step.
...
The prod portal is currently exposed via an **OKD `Route` with inline TLS** — the certificate and private key are embedded in the Route resource itself. That cert is a frozen copy with a fixed expiry and no auto-rotation.
...
4. Onboarding doc step 8 reads "✅ Ingress + shared wildcard Secret".
