...
describe('hostOf', () => {
  it('extracts host from ssh URLs (ignoring user + port)', () => {
    expect(hostOf('ssh://git@bitbucket.spb.openwaygroup.com:7999/proj/repo.git')).toBe(
      'bitbucket.spb.openwaygroup.com',
    )
    expect(hostOf('ssh://bitbucket.spb.openwaygroup.com/proj/repo.git')).toBe(
      'bitbucket.spb.openwaygroup.com',
    )
  })
  it('extracts host from https URLs and lowercases', () => {
    expect(hostOf('https://Bitbucket.SPB.openwaygroup.com/')).toBe('bitbucket.spb.openwaygroup.com')
  })
...
describe('isVcsHostSupported', () => {
  const git = 'https://bitbucket.spb.openwaygroup.com'
  it('accepts a matching host (case/port/user-insensitive)', () => {
    expect(isVcsHostSupported('ssh://git@bitbucket.spb.openwaygroup.com:7999/p/r.git', git)).toBe(true)
  })
