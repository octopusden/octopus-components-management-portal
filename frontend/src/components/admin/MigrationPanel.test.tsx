...
beforeEach(() => {
  ...
  // TC resync / TC validation idle by default; cross-disable tests override
  // with a RUNNING fixture to assert the components button gets blocked.
...
describe('MigrationPanel — cross-disable when history migration is RUNNING', () => {
  ...
  it('disables Run migration when TC resync is RUNNING and shows a helper hint', () => {
    useAdminMode.setState({ enabled: true })
    mockUseTeamCityResyncJob.mockReturnValue({
  ...
  it('disables Run migration when TC validation is RUNNING and shows a helper hint', () => {
    useAdminMode.setState({ enabled: true })
    mockUseTeamCityValidationJob.mockReturnValue({
