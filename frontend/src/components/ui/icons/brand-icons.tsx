...
// match the §7.0.6 mockup (`prototypes/index.html`). lucide-react has no
// Atlassian/JetBrains-branded equivalents, so the SVG paths are inlined
// here. Brand colors (Atlassian blue, JetBrains cyan) are intentional and
// retained across themes — they're trademarked vendor colors, not a
// design-system token.
//
// Each component accepts a `className` so callers can size it via
// utilities like `h-4 w-4` (matching the existing IconLink convention).
...
export function JiraIcon({ className }: BrandIconProps) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill={ATLASSIAN_BLUE} aria-hidden="true">
...
export function TeamCityIcon({ className }: BrandIconProps) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill={JETBRAINS_CYAN} aria-hidden="true">
