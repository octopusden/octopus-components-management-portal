...
/**
 * Full dictionary of `labels` reference values, sourced from the master table
 * (NOT the in-use M:N junction surfaced by `/components/meta/labels`). Used
 ...
 */
export function useLabelsDictionary() {
  return useQuery({
    queryKey: ['meta', 'labels-dictionary'],
    queryFn: async () => {
      try {
        return await api.get<string[]>('/components/meta/labels/dictionary')
      } catch (e) {
