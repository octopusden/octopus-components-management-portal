...
- **Can't verify end-to-end without a running CRS instance.** CRS's `rms-registered-build-params` branch has since merged to `main` — `v4.json`/`schema.d.ts` are re-vendored and carry `registeredBuildParameters`, and this design's read-side assumptions (esp. Decision 1's list-view claim) are now confirmed against the actual merged mapper, not just CRS's own docs. What remains untested is a live CRS instance actually returning the `409`/`503` responses and real RMS-backed data — implementation is still proceeding against hand-written fixtures/mocks for that part.
