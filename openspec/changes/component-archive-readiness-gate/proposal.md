...
- **Portal writes the blocking sentence.** CRS sends no reason on a blocking `FAILED` entry — the outcome and the target kind are the whole answer it gives. The wording that tells someone a repository is still not archived is Portal's to author, per target kind.
